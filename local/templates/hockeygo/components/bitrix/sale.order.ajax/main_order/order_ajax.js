	...
	BX.Sale.OrderAjaxComponent = {
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'col-sm-7 bx-soa-pp-item-container'}}),
				...
				paySystemItemNode = this.createPaySystemItem(this.paySystemPagination.currentPage);
				paySystemItemsContainer.appendChild(paySystemItemNode);
		...
		for (i = 0; i < item.length; i++)
			...
			var paySystemId = parseInt(item[i].ID)
			if(i == 0)
				var checked = item[i].CHECKED == 'Y'
			else
				var checked = item[i].CHECKED == 'N'

					item_pay_system = BX.create('OPTION', {
						props: {
					...
					label.appendChild(item_pay_system);
