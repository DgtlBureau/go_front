...
<main class="shop catalog">
    ...
    <div class='container'>
        ...
        if ($elementId > 0) {
            ...
            $db_props = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], $elementId, array("sort" => "asc"), array("CODE" => "PERSONAL"));
            while ($ar_props = $db_props->Fetch())
                if (!empty($ar_props["VALUE"]))
                    $arElements[] = IntVal($ar_props["VALUE"]);

            if (!empty($arElements)) {
                $GLOBALS[$arParams["FILTER_NAME"]] = ['ID' => $arElements];

                if (!isset($arParams['DETAIL_SHOW_VIEWED']) || $arParams['DETAIL_SHOW_VIEWED'] != 'N') {
                    ?>
