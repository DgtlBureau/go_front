...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function (parameters) {
            ...
            console.log('init', this.result.ORDER_PROP)
            ...
            this.profileNode = BX(parameters.profilePropsBlockId);
            this.otherNode = BX(parameters.otherPropsBlockId);
        ...
        sendRequest: function (action, actionData) {
            console.log('sendRequest', action, actionData)
        ...
        refreshOrder: function (result) {
            console.log('refreshOrder', result)
        ...
        showError: function (node, msg, border) {
            ...
            console.log('showError', node, msg)

            // BX.addClass(node, 'error-input');
        ...
        showErrors: function (errors, scroll, showAll) {
            ...
            for (k in errors) {
                ...
                switch (k.toUpperCase()) {
                    case 'MAIN':
                        console.log('showError - MAIN', blockErrors);
                    ...
                    case 'REGION':
                        if (showAll || this.regionBlockNode.getAttribute('data-visited') === 'true') {
                            console.log('showError - REGION', blockErrors);
                    ...
                    case 'DELIVERY':
                        if (showAll || this.deliveryBlockNode.getAttribute('data-visited') === 'true') {
                            ...
                            console.log('showError - DELIVERY', blockErrors);
                    ...
                    case 'PAY_SYSTEM':
                        if (showAll || this.paySystemBlockNode.getAttribute('data-visited') === 'true') {
                            ...
                            console.log('showError - PAY_SYSTEM', blockErrors);
                    ...
                    case 'PROPERTY':
                        if (showAll || this.propsBlockNode.getAttribute('data-visited') === 'true') {
                            ...
                            console.log('showError - PROPERTY', blockErrors);
        ...
        showDeliveryItemsPage: function (page) {
            ...
            console.log('deliveryBlockNode', this.deliveryBlockNode)

            // deliveryItemsContainer = this.deliveryBlockNode.querySelector('.bx-soa-pp-item-container');
            deliveryItemsContainer = BX('delivery-items-block');
        ...
        showPickUpItemsPage: function (page) {
            console.log('showPickUpItemsPage', page)
        ...
        clickOrderSaveAction: function (event) {
            ...
            console.log('clickOrderSaveAction', this.isValidForm())
        ...
        //             if (state && sections[i + 1] && this.checkBlockErrors(sections[i]) && (
        ...
        editSections: function (sections) {
            ...
            if (altProperty) {
                ...
                input = BX.create('input', {
                    props: {
                        ...
                        autocomplete: 'city',
                        className: 'order__input required',
        ...
        editActivePaySystemBlock: function (activeNodeMode) {
            ...
            if (this.initialized.paySystem) {
                ...
                paySystemContent = BX('payment-systems');
        ...
        editPaySystemItems: function (paySystemNode) {
            ...
            var paySystemSelectNode = this.createPaySystemItem(this.paySystemPagination.currentPage);

            paySystemNode.appendChild(paySystemSelectNode);
        },

        createPaySystemItem: function (item) {
            var selectNode = BX.create('SELECT', {
                props: {}
            });

            for (let i = 0; i < item.length; i++) {
                let paySystemId = parseInt(item[i].ID)
                if (i == 0)
                    var checked = item[i].CHECKED == 'Y'
                else
                    var checked = item[i].CHECKED == 'N'

                let item_pay_system = BX.create('OPTION', {
                    props: {
                ...
                selectNode.appendChild(item_pay_system);

                if (checked)
                    BX.addClass(item_pay_system, 'bx-selected');
            }
        ...
        // OFF - доп. информация о типе оплаты
        ...
        getSelectedPaySystem: function () {
            var paySystemCheckbox = this.paySystemBlockNode.querySelector('option[data-name="PAY_SYSTEM_ID"]:checked'),
                currentPaySystem = null, paySystemId, i;

            if (!paySystemCheckbox)
                paySystemCheckbox = this.paySystemHiddenBlockNode.querySelector('option[data-name="PAY_SYSTEM_ID"]:checked');
        ...
        editActiveDeliveryBlock: function (activeNodeMode) {
            ...
            if (this.initialized.delivery) {
                ...
                this.editDeliveryInfo(deliveryContent);
        ...
        editDeliveryItems: function (deliveryNode) {
            ...
            var
                // deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-7 bx-soa-pp-item-container'}}),
                deliveryItemNode, k;
        ...
        editDeliveryInfo: function (deliveryNode) {
            ...
            // BX('title-delivery-block').innerHTML = 'Способ доставки - ' + currentDelivery.NAME;
            ...
            deliveryInfoContainer = BX.create('DIV', {
                props: {className: 'bx-soa-pp-company'},
                children: [ clear, extraServices, infoList]
            });


            deliveryNode.appendChild(deliveryInfoContainer);
            deliveryNode.appendChild(title);
        ...
        createDeliveryItem: function (item) {
            ...
            var inputNode = BX.create('INPUT', {
                props: {
                    ...
                    name: 'DELIVERY_ID',
                    type: 'checkbox',
                    className: 'delivery radio-btn',
                    value: deliveryId,
                    checked: checked
                }
            });

            var labelNode = BX.create('LABEL', {
                props: {for: 'ID_DELIVERY_ID_' + deliveryId},
            })
            ...
            var labelNodes = [inputNode, labelNode, title],
                deliveryCached = this.deliveryCachedInfo[deliveryId],
                logotype, label, itemNode, logoNode;

            var divNode = BX.create('DIV', {
                props: {
                    className: 'order__radio-txt'
                },
                children: labelNodes
            });
        ...
        editPickUpList: function (isNew) {
            ...
            console.log('editPickUpList', selectedDelivery)
        ...
        editActivePropsBlock: function (activeNodeMode) {
            ...
            if (this.initialized.props) {
                ...
                BX.cleanNode(this.profileNode);
                BX.cleanNode(this.otherNode);
        ...
        editPropsItems: function () {
            if (!this.result.ORDER_PROP || !this.propertyCollection)
                return;

            console.log('ORDER_PROP', this.result.ORDER_PROP)
            ...
            while (group = groupIterator()) {
                ...
                while (property = propsIterator()) {
                    ...
                    let arrProfileProps = ['NAME', 'SECONDNAME', 'PHONE', 'EMAIL'];

                    if (arrProfileProps.includes(property.getSettings().CODE))
                        this.getPropertyRowNode(property, this.profileNode, false);
                    else {
                        console.log('property', property.getSettings())

                        // if (property.getSettings().TYPE == 'LOCATION')
                        //     continue;


                        this.getPropertyRowNode(property, this.otherNode, false);
                        // this.getPropertyRowNode(property, this.regionBlockNode, false);

                    }
                }
            }

            this.editPropsComment(this.otherNode);
        },
        ...
        getPropertyRowNode: function (property, propsItemsContainer, disabled) {
            var propsItemNode = BX.create('DIV', {
                ...
                props: {className: 'order__inputs-item'},
                // html: textHtml
            });

            let arLongFields = ['IPOLSDEK_STORE'];

            if (arLongFields.includes(property.getSettings().CODE))
                BX.addClass(propsItemNode, 'long-input');
            else {
                if (propsItemsContainer == this.otherNode)
                    BX.addClass(propsItemNode, 'input');
            }
        ...
        insertLocationProperty: function (property, propsItemNode, disabled) {
            ...
            propsItemNode.className = 'order__input';
            ...
            if (property.getId() in this.locations) {
                if (disabled) {
                    ...
                    for (i = 0; i < propRow.length; i++) {
                        ...
                        insertedLoc.innerHTML = currentLocation.HTML;


                        // this.otherNode
                        propsItemNode.appendChild(insertedLoc);
        ...
        insertDateProperty: function (property, propsItemNode, disabled) {
            ...
            if (disabled) {
                ...
                propContainer = BX.create('DIV', {props: {className: 'order__inputs-item'}});
        ...
        insertStringProperty: function (property, propsItemNode, disabled) {
            ...
            if (disabled) {
                ...
                var errorContainer = BX.create('DIV', {props: {className: 'error-txt hide'}});

                property.appendTo(propsItemNode);
                propsItemNode.appendChild(errorContainer);
        ...
        alterProperty: function (settings, propContainer) {
            ...
            for (i = 0; i < inputs.length; i++) {
                ...
                BX.addClass(inputs[i], 'order__input');
                if (settings.REQUIRED == 'Y') {
                    inputs[i].placeholder += '*';
                    BX.addClass(inputs[i], 'required');
                }
            }
            ...
            if (add.length) {
                ...
                BX.bind(add, 'click', BX.delegate(function (e) {
                    var target = e.target || e.srcElement,
                        targetContainer = BX.findParent(target, {tagName: 'div', className: 'order__inputs-item'}),
                    ...
                    if (textInputs.length) {
                        ...
                        BX.addClass(textInputs[textInputs.length - 1], 'order__input required');
        ...
        isValidRegionBlock: function () {
            ...
            var regionProps = this.otherNode.querySelectorAll('.order__input[data-property-id-row]'),
                regionErrors = [],
            ...
            console.log('isValidRegionBlock', regionErrors)
            regionErrors = [];
        ...
        isValidPropertiesBlock: function (excludeLocation) {
            ...
            for (i = 0; i < props.length; i++) {
                ...
                propContainer = props[i].querySelector('.order__inputs-item');
                if (propContainer) {
                    arProperty = this.validation.properties[id];
        ...
        showValidationResult: function (inputs, errors) {
            console.log('showValidationResult', inputs, errors)

            if (!inputs || !inputs.length || !errors)
                return;

            var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
                parentDiv = BX.findParent(input0, {tagName: 'DIV', className: 'order__input'}),
                tooltipId, inputDiv, i;

            if (!parentDiv)
                parentDiv = BX.findParent(input0, {tagName: 'DIV', className: 'order__inputs-item'});
            ...
            if (errors.length)
                this.showErrorTooltip(tooltipId, inputDiv, errors.join('<br>'));
            else
        ...
        editPropsComment: function (propsNode) {
            let commentNode = BX.create('TEXTAREA', {
                props: {
                    id: 'orderDescription',
                    cols: '4',
            ...
            propsNode.appendChild(commentNode);
        },
        ...
        editTotalBlock: function () {
            ...
            this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_SUMMARY'), priceHtml, params)); // сумма заказа
            ...
            this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_QUANTITY'), total.BASKET_POSITIONS, {total: true}));

            if (this.result.DELIVERY.length) {
                this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_DELIVERY'), deliveryValue, params)); // Доставка
            }
            ...
            if (this.options.showPayedFromInnerBudget) {
                ...
                this.totalInfoBlockNode.appendChild(this.createTotalUnit2(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED, {total: true})); // итого
            }
            ...
            if (!this.result.SHOW_AUTH && !BX('order-submit-btn')) {
                // this.totalInfoBlockNode.appendChild(BX.create('A', {
                BX('bx-soa-paysystem').appendChild(BX.create('A', {
                    props: {
                        href: 'javascript:void(0)',
                    ...
                    children: [
                        BX.create('DIV', {
                            props: {
                                className: 'order__payment-btn-txt'
                            },
        ...
        createTotalUnit: function (name, value, params) {
            ...
            var totalValue = BX.create('DIV', {
                props: {className: 'order__payment-price'},
                children: [
                    BX.create('DIV', {
                        props: {
                            className: 'order__payment-txt'
                        },
                        html: name
                    }),
                    BX.create('DIV', {
        ...
        createTotalUnit2: function (name, value, params) {
            name = name || '';
            value = value || '';
            params = params || {};

            var totalValue = BX.create('DIV', {
                props: {className: 'order__payment-price paymant-summ'},
                children: [
                    BX.create('DIV', {
                        props: {
                            className: 'order__payment-txt'
